...
class IndirectTestProvider extends ChangeNotifier {
  ...
  Future<String> getInfoTest(String email)async {
    var collection = FirebaseFirestore.instance.collection("test indirecto");
  ...
  bool verificarSecuencia(String secuenciaUsuario,String email){
    ...
    if(_isRuning){
      ...
      if(secuencia == secuenciaUsuario){
        ...
        FirebaseFirestore.instance.collection("test indirecto").doc(emailConfirm).set({"test":_puntos});
        _isRuning = false;
        numeros = 2;
        _puntos = 0;
        return false;
